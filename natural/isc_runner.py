...
def get_subject_roi(data_dict, roi_num):
    ...
    for sub, value in data_dict.items():
        try:
            ...
            roy = int(ROIS.iloc[roi_num])
            sub_rois[sub] = value[roy]
    return (pd.DataFrame(sub_rois))
 
def fcisc(inpu,outpu,method):
	try:
		stats = isc(inpu, n_bootstraps=5000, metric='median', method='%s'%(method),return_bootstraps = True)
	except ValueError:
		test = inpu.dropna(axis=1)
		print('dropping NAN')
		stats = isc(test, n_bootstraps=5000, metric='median', method='%s'%(method), return_bootstraps = True)
...
# Add an argument
	...
	pars.add_argument('--method', type=str, required=True)
# Parse
	parser = pars.parse_args()
	print(parser)
# get data
	df = pd.concat([get_subject_roi(data_dict[parser.condition]['run-1'], parser.ROI), get_subject_roi(data_dict[parser.condition]['run-2'], parser.ROI)], ignore_index=True) 
# make dicts and run code
	stats = fcisc(df, os.path.join(outpath,'isc_ROI-%s_%s'%(parser.ROI, parser.condition)),parser.method)
# save pickle
	packit = {'%s'%parser.condition:{'%s_ROI'%parser.ROI:{'stats':stats, 'df':df,'method':parser.method}}}
	adillyofapickle(outpath,packit, '%s_%s_%s'%(parser.condition,parser.ROI,parser.method))
